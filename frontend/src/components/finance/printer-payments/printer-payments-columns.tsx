import { FinancePrinterPaymentItem } from "@/types/finance.types";

import { ColumnDef } from "@tanstack/react-table";
import { CircleAlertIcon } from "lucide-react";
import dayjs from "dayjs";

import PrinterPaymentsActionsMenu from "./printer-payments-actions-menu";

import { cn } from "@/lib/utils";
import { PAYMENT_STATUS_COLORS } from "@/constants/orders.constants";

export const getPrinterPaymentsColumns = (
  t: (key: string) => string
): ColumnDef<FinancePrinterPaymentItem>[] => [
  {
    id: "order_important",
  ...
  },
  {
    ...
    header: () => (
      <div className="whitespace-normal break-words text-center">
        {t("finance.table_headers.all_finance.print")}
      </div>
    ),

    cell: ({ row }) => {
      const printer = row.original.printer;
  ...
  },
  {
    ...
    cell: ({ row }) => {
      const printingCost = row.original.printing_cost;

      if (!printingCost) {
        return <div className="text-center">-</div>;
      }
      return <div className={cn("text-center")}>{printingCost}</div>;
    },
    size: 80,
  },
  {
  ...
  {
    accessorKey: "printing_payment_date",
    header: () => (
      <div className="whitespace-normal break-words text-center">
  ...
  },
  {
    ...
    cell: ({ row }) => {
      return (
        <div className="text-center">
          <PrinterPaymentsActionsMenu id={row.original.order_id} />
        </div>
      );
    },
    size: 28,
