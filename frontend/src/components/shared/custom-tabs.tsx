...
const CustomTabs = ({ tabsOptions, isModal }: CustomTabsProps) => {
  const firstTab = tabsOptions[0];

  const [selected, setSelected] = useState<TabOption>(firstTab);

  const handleChange = (value: string) => {
    if (selected.value === value) return;

    const selectedTab = tabsOptions.find((t) => t.value === value);
    if (!selectedTab) {
      return;
    }
